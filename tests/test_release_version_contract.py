...
class ReleaseVersionContractTests(unittest.TestCase):
    ...
    def test_app_runtime_and_display_versions_are_1_4_1(self) -> None:
        self.assertRegex(self.constants, r'(?m)^APP_VERSION\s*=\s*"1\.4\.1"$')
        self.assertRegex(
            self.constants,
            r'(?m)^APP_DISPLAY_VERSION\s*=\s*"1\.4\.1"$',
        )
        self.assertRegex(
            self.constants,
    ...
    def test_windows_exe_version_metadata_matches_1_4_1(self) -> None:
        self.assertIn("filevers=(1, 4, 1, 0)", self.version_info)
        self.assertIn("prodvers=(1, 4, 1, 0)", self.version_info)
        self.assertIn("StringStruct('FileVersion', '1.4.1')", self.version_info)
        self.assertIn("StringStruct('ProductVersion', '1.4.1')", self.version_info)

    def test_inno_installer_version_matches_1_4_1(self) -> None:
        self.assertRegex(
            self.installer,
            r'(?m)^#define\s+MyAppVersion\s+"1\.4\.1"$',
        )
    ...
    def test_packaging_checklist_targets_1_4_1_hotfix(self) -> None:
        first_line = self.checklist.splitlines()[0]
        self.assertEqual(
            first_line,
            "RR-V 1.4.1 community beta hotfix release checklist",
        )
        self.assertIn("Clean upgrade regression: 1.4.0 -> 1.4.1", self.checklist)
        self.assertIn("Fresh-install installer regression (1.4.1 hotfix)", self.checklist)
        self.assertIn("installer-output\\RR-V_Setup_1.4.1.exe", self.checklist)
